...
/* Objc section enum */
enum objc_section_enum {
#define DEF_SECTION(NAME, FLAGS, DIRECTIVE, OBJC) NAME,
#include "objc-sections.def"
#undef DEF_SECTION
  NUM_OBJC_SECTIONS
};

/* Objc section array */
section * objc_sections[NUM_OBJC_SECTIONS];
...
output_objc_section_asm_op (const void *directive)
{
  ...
  if (! been_here)
    {
      static const enum objc_section_enum tomarkv2[] =
	{
	...
	for (i = 0; i < ARRAY_SIZE (tomarkv2); i++)
	  switch_to_section (objc_sections[tomarkv2[i]]);
    }
...
{
  if (objc_internal_variable_name (decl)) {
	...
	;
        ...
        return objc_sections[objc_v2_classrefs_section];
      else if (!strncmp (name, "_OBJC_CLASSLIST_SUP_REFS_", 25))
        return objc_sections[objc_v2_super_classrefs_section];
      else if (!strncmp (name, "l_objc_msgSend", 14))
        return objc_sections[objc_v2_message_refs_section];
      else if (!strncmp (name, "_OBJC_LABEL_CLASS_", 18))
	return objc_sections[objc_v2_classlist_section];
      else if (!strncmp (name, "l_OBJC_LABEL_PROTOCOL_", 22))
        return objc_sections[objc_v2_protocollist_section];
      else if (!strncmp (name, "_OBJC_LABEL_CATEGORY_", 21))
        return objc_sections[objc_v2_categorylist_section];
      else if (!strncmp (name, "_OBJC_LABEL_NONLAZY_CLASS_", 26))
        return objc_sections[objc_v2_nonlazy_class_section];
      else if (!strncmp (name, "_OBJC_LABEL_NONLAZY_CATEGORY_", 29))
        return objc_sections[objc_v2_nonlazy_category_section];
      else if (!strncmp (name, "l_OBJC_PROTOCOL_REFERENCE_", 26))
        return objc_sections[objc_v2_protocolrefs_section];
      else if (!strncmp (name, "_OBJC_SELECTOR_REFERENCES", 25))
        return objc_sections[objc_v2_selector_refs_section];
    }
  }
  const char *sname;
  switch (categorize_decl_for_section (decl, reloc))
